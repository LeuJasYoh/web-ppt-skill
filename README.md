...
- **Vue Vapor**：Vue 3.6 的编译期渲染模式稳定后切换，产物更小更快，模板代码不用重写
- **外部资源模式**：exe 只内嵌骨架（样式、动效、字体），视频等大文件放同级的 `media/` 目录，缺文件时显示占位框；视频起步 H.264/MP4
- **后端服务**：deck 云端托管与分享链接、远程翻页遥控、主题市场；后端顺带做视频格式扩展（H.265 / AV1 转码），前端只管 `<video>`
