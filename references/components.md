...
4. 字体/图标/动效全部来自 npm 包（@fontsource、lucide、motion），构建期内嵌、离线可用——
   不要按旧习惯手写任何 CDN <script> 或 Google Fonts 链接。
-->


# 组件参考 · Components

这是 `guizang-ppt-skill` skill 的组件手册。magazine.css 已经定义好了所有样式，这里只写"这个组件长什么样、怎么用"。
...
每一页都是一个 `<section class="slide ...">`。主题由 **class 推断**（`src/style.js` 读取 `light` / `dark` 类切换 WebGL 背景；`data-theme` 属性可选，写了会优先生效）。示例页都只写 class——保持一致即可，不要自造主题名。

```html
<section class="slide light">        <!-- 浅色页 -->
<section class="slide dark">         <!-- 深色页 -->
<section class="slide light hero">   <!-- Hero 页：浅色 + 薄遮罩透出 WebGL -->
<section class="slide dark hero">    <!-- Hero 页：深色 + 薄遮罩 -->
```
...
6. **用户原始截图比例不合适时**：给 `.frame-img` 加 `.fit-contain` 保留原始比例,容器用登记标准比例(16/10 / 4/3 / 3/2 / 1/1 / 16/9)统一;只有截图太长、太窄或需要重构信息时,才重新生成"截图再设计 / UI 情景图"。
...
**严禁使用 emoji**。用 Lucide 图标库——**npm 包 `lucide` 已在依赖里,构建期内嵌,离线可用**。App.vue 挂载后会自动调用 `createIcons` 把所有 `<i data-lucide>` 物化为 SVG,页面里只管写标签,**不要**手写 CDN `<script>`（离线 exe 里会 404）：
...
整套 deck 默认开启翻页入场动画,由 **Motion One 驱动**——npm 包 `motion` 在构建期打包进产物,**彻底离线可用**,没有 CDN 回落逻辑。
...
- **内容静态时也不显示?** 开浏览器控制台看报错;再核对 recipe 挂点(如 quote 页的 `data-anim="line"`)是否写对
