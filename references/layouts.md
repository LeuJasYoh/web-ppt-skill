...
- grid 容器默认 `align-items:start`（已在 magazine.css 设置），图片自然贴到 cell 顶端
...
**核心机制**:动效引擎在 `src/composables/useDeckMotion.js` + `recipesMagazine.js`(npm 包 motion 驱动)。所有带 `data-anim` 的元素初始不可见,翻到当前页时逐个淡入。
